...
static void init_oledView(void)
{
    int8_t x = 0;
    int8_t y = 0;
    int8_t z = 0;
...
static void update_oledView(void)
{
    ...
    if ((now - lastUpdate) < OLED_REFRESH_INTERVAL_MS) {
        return;
    }
    lastUpdate = now;
...
int main (void) {
    ...
    while (1) {
        ...
        /* OLED sensors are updated continuously and independently. */
        update_oledView();
